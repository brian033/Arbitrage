...
class WeightedPool {
    ...
    _exactTokenInForTokenOut(poolPairData, amount) {
        ...
        try {
            const amt = _calcOutGivenIn$3(takeToPrecision18(balanceIn, decimalsIn).toBigInt(), poolPairData.weightIn.toBigInt(), takeToPrecision18(balanceOut, decimalsOut).toBigInt(), poolPairData.weightOut.toBigInt(), bignumber.parseFixed(amount.dp(18, 1).toString(), 18).toBigInt(), poolPairData.swapFee.toBigInt());
            // return human scaled
    ...
    _tokenInForExactTokenOut(poolPairData, amount) {
        ...
        const decimalsOut = poolPairData.decimalsOut;
        try {
            const amt = _calcInGivenOut$3(takeToPrecision18(balanceIn, decimalsIn).toBigInt(), poolPairData.weightIn.toBigInt(), takeToPrecision18(balanceOut, decimalsOut).toBigInt(), poolPairData.weightOut.toBigInt(), bignumber.parseFixed(amount.dp(18, 1).toString(), 18).toBigInt(), poolPairData.swapFee.toBigInt());
            // return human scaled
            const amtOldBn = bnum(amt.toString());
            return scale(amtOldBn, -18);
        }
...
const _MAX_IN_RATIO$1 = bignumber.parseFixed('0.3', 18);
const _MAX_OUT_RATIO$1 = bignumber.parseFixed('0.3', 18);
...
function _calculateQuadraticTerms(balances, sqrtAlpha, sqrtBeta) {
    ...
    const bSq2 = divDown$1(mulDown$1(mulDown$1(mulDown$1(balances[0], balances[1]), constants.WeiPerEther.mul(2)), sqrtAlpha), sqrtBeta);
...
function _calcOutGivenIn$1(balanceIn, balanceOut, amountIn, virtualParamIn, virtualParamOut) {
    ...
    const virtInOver = balanceIn.add(mulUp$1(virtualParamIn, constants.WeiPerEther.add(2)));
    const virtOutUnder = balanceOut.add(mulDown$1(virtualParamOut, constants.WeiPerEther.sub(1)));
...
function _getNormalizedLiquidity$1(balances, virtualParamIn, swapFee) {
    ...
    const afterFeeMultiplier = constants.WeiPerEther.sub(swapFee);
...
class Gyro2Pool {
    ...
    getLimitAmountSwap(poolPairData, swapType) {
        if (swapType === exports.SwapTypes.SwapExactIn) {
            return bnum(bignumber.formatFixed(mulDown$1(poolPairData.balanceIn, this.MAX_IN_RATIO), poolPairData.decimalsIn));
        }
        else {
            return bnum(bignumber.formatFixed(mulDown$1(poolPairData.balanceOut, this.MAX_OUT_RATIO), poolPairData.decimalsOut));
        }
    }
    // Updates the balance of a given token for the pool
    updateTokenBalanceForPool(token, newBalance) {
...
function _calculateCubicStartingPoint(a, mb, mc) {
    ...
    const lmin = divUp(mb, a.mul(3)).add(divUp(newtonSqrt(radic, bignumber.BigNumber.from(5)), a.mul(3)));
...
class Gyro3Pool {
    ...
    static fromPool(pool) {
        ...
        if (bignumber.parseFixed(pool.root3Alpha, 18).lte(0) ||
    ...
    parsePoolPairData(tokenIn, tokenOut) {
        ...
        const poolPairData = {
            ...
            balanceIn: bignumber.parseFixed(balanceIn, decimalsIn),
            balanceOut: bignumber.parseFixed(balanceOut, decimalsOut),
            balanceTertiary: bignumber.parseFixed(balanceTertiary, decimalsTertiary),
            swapFee: this.swapFee,
        };
        return poolPairData;
    }
    ...
    getLimitAmountSwap(poolPairData, swapType) {
        if (swapType === exports.SwapTypes.SwapExactIn) {
            return bnum(bignumber.formatFixed(mulDown(poolPairData.balanceIn, this.MAX_IN_RATIO), poolPairData.decimalsIn));
        }
        else {
            return bnum(bignumber.formatFixed(mulDown(poolPairData.balanceOut, this.MAX_OUT_RATIO), poolPairData.decimalsOut));
        }
    }
    // Updates the balance of a given token for the pool
    updateTokenBalanceForPool(token, newBalance) {
