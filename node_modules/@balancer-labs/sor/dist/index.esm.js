...
class WeightedPool {
    ...
    _exactTokenInForTokenOut(poolPairData, amount) {
        ...
        try {
            const amt = _calcOutGivenIn$3(takeToPrecision18(balanceIn, decimalsIn).toBigInt(), poolPairData.weightIn.toBigInt(), takeToPrecision18(balanceOut, decimalsOut).toBigInt(), poolPairData.weightOut.toBigInt(), parseFixed(amount.dp(18, 1).toString(), 18).toBigInt(), poolPairData.swapFee.toBigInt());
            // return human scaled
    ...
    _tokenInForExactTokenOut(poolPairData, amount) {
        ...
        const decimalsOut = poolPairData.decimalsOut;
        try {
            const amt = _calcInGivenOut$3(takeToPrecision18(balanceIn, decimalsIn).toBigInt(), poolPairData.weightIn.toBigInt(), takeToPrecision18(balanceOut, decimalsOut).toBigInt(), poolPairData.weightOut.toBigInt(), parseFixed(amount.dp(18, 1).toString(), 18).toBigInt(), poolPairData.swapFee.toBigInt());
            // return human scaled
            const amtOldBn = bnum(amt.toString());
            return scale(amtOldBn, -18);
        }
...
function _spotPriceAfterSwapExactTokenInForTokenOut$3(amount, poolPairData) {
    ...
    ans = ONE.div(ans.times(WeiPerEther.sub(swapFee).toString()).div(WeiPerEther.toString()));
    return ans;
}
// PairType = 'token->token'
// SwapType = 'swapExactOut'
...
function _spotPriceAfterSwapTokenInForExactBPTOut$2(amount, poolPairData) {
    ...
    let ans = _poolDerivativesBPT$2(amp, balances, bnum(formatFixed(balanceOut, decimalsOut)).plus(amount), tokenIndexIn, true, true, true);
...
function _calculateInvariant$2(amp, balances, roundUp) {
    ...
    for (let i = 0; i < 255; i++) {
        ...
        for (let j = 1; j < numTokens; j++) {
            P_D = MathSol.div(MathSol.mul(MathSol.mul(P_D, balances[j]), BigInt(numTokens)), invariant, roundUp);
        }
        prevInvariant = invariant;
        invariant = MathSol.div(MathSol.mul(MathSol.mul(BigInt(numTokens), invariant), invariant) +
            MathSol.div(MathSol.mul(MathSol.mul(ampTimesTotal, sum), P_D), AMP_PRECISION, roundUp), MathSol.mul(BigInt(numTokens + 1), invariant) +
            // No need to use checked arithmetic for the amp precision, the amp is guaranteed to be at least 1
            MathSol.div(MathSol.mul(ampTimesTotal - AMP_PRECISION, P_D), AMP_PRECISION, !roundUp), roundUp);
...
const _MAX_IN_RATIO$1 = parseFixed('0.3', 18);
const _MAX_OUT_RATIO$1 = parseFixed('0.3', 18);
...
function _sqrt(input, tolerance) {
    ...
    if (!(guessSquared.lte(input.add(mulUp$1(guess, tolerance))) &&
...
function _calculateQuadraticTerms(balances, sqrtAlpha, sqrtBeta) {
    ...
    let bSquare = mulDown$1(mulDown$1(mulDown$1(balances[0], balances[0]), sqrtAlpha), sqrtAlpha);
    const bSq2 = divDown$1(mulDown$1(mulDown$1(mulDown$1(balances[0], balances[1]), WeiPerEther.mul(2)), sqrtAlpha), sqrtBeta);
    const bSq3 = divDown$1(mulDown$1(balances[1], balances[1]), mulUp$1(sqrtBeta, sqrtBeta));
...
function _calcOutGivenIn$1(balanceIn, balanceOut, amountIn, virtualParamIn, virtualParamOut) {
    /**********************************************************************************************
        ...
        **********************************************************************************************/
    if (amountIn.gt(mulDown$1(balanceIn, _MAX_IN_RATIO$1)))
    ...
    const virtInOver = balanceIn.add(mulUp$1(virtualParamIn, WeiPerEther.add(2)));
    const virtOutUnder = balanceOut.add(mulDown$1(virtualParamOut, WeiPerEther.sub(1)));
    const amountOut = divDown$1(mulDown$1(virtOutUnder, amountIn), virtInOver.add(amountIn));
    if (amountOut.gte(balanceOut))
        throw new Error('ASSET_BOUNDS_EXCEEDED');
    // This in particular ensures amountOut < balanceOut.
    if (amountOut.gt(mulDown$1(balanceOut, _MAX_OUT_RATIO$1)))
...
function _calcInGivenOut$1(balanceIn, balanceOut, amountOut, virtualParamIn, virtualParamOut) {
    ...
    const virtInOver = balanceIn.add(mulUp$1(virtualParamIn, WeiPerEther.add(2)));
    const virtOutUnder = balanceOut.add(mulDown$1(virtualParamOut, WeiPerEther.sub(1)));
    const amountIn = divUp$1(mulUp$1(virtInOver, amountOut), virtOutUnder.sub(amountOut));
    if (amountIn.gt(mulDown$1(balanceIn, _MAX_IN_RATIO$1)))
...
function _derivativeSpotPriceAfterSwapTokenInForExactTokenOut$1(balances, inAmount, outAmount, virtualParamIn, virtualParamOut, swapFee) {
    ...
    const numerator = virtIn.add(mulDown$1(afterFeeMultiplier, inAmount)); // x' + (1 - s) * dx
    const virtOut = balances[1].add(virtualParamOut); // y + virtualParamY = y'
    const denominator = mulDown$1(virtOut.sub(outAmount), virtOut.sub(outAmount)); // (y' + dy)^2
...
// /////////
function _getNormalizedLiquidity$1(balances, virtualParamIn, swapFee) {
    /**********************************************************************************************
        // x = balanceIn             x' = x +  virtualParamX                                         //
        // s = swapFee                                                                               //
        //                                                     1                                     //
    ...
    const afterFeeMultiplier = WeiPerEther.sub(swapFee);
    const normalizedLiquidity = divDown$1(virtIn, afterFeeMultiplier);
...
class Gyro2Pool {
    ...
    getNormalizedLiquidity(poolPairData) {
        ...
        const normalizedBalances = _normalizeBalances$1(balances, poolPairData.decimalsIn, poolPairData.decimalsOut);
        const invariant = _calculateInvariant$1(normalizedBalances, poolPairData.sqrtAlpha, poolPairData.sqrtBeta);
        const [virtualParamIn] = _findVirtualParams(invariant, poolPairData.sqrtAlpha, poolPairData.sqrtBeta);
        const normalisedLiquidity = _getNormalizedLiquidity$1(normalizedBalances, virtualParamIn, poolPairData.swapFee);
        return bnum(formatFixed(normalisedLiquidity, 18));
    }
    getLimitAmountSwap(poolPairData, swapType) {
        if (swapType === SwapTypes.SwapExactIn) {
            return bnum(formatFixed(mulDown$1(poolPairData.balanceIn, this.MAX_IN_RATIO), poolPairData.decimalsIn));
        }
        else {
            return bnum(formatFixed(mulDown$1(poolPairData.balanceOut, this.MAX_OUT_RATIO), poolPairData.decimalsOut));
        }
    }
    // Updates the balance of a given token for the pool
    updateTokenBalanceForPool(token, newBalance) {
...
// Swap limits: amounts swapped may not be larger than this percentage of total balance.
const _MAX_IN_RATIO = parseFixed('0.3', 18);
const _MAX_OUT_RATIO = parseFixed('0.3', 18);
// SQRT constants
...
function _calculateCubicStartingPoint(a, mb, mc) {
    ...
    const lmin = divUp(mb, a.mul(3)).add(divUp(newtonSqrt(radic, BigNumber$1.from(5)), a.mul(3)));
...
// ///  Normalized Liquidity
// /////////
function _getNormalizedLiquidity(balances, virtualParamIn, swapFee) {
    /**********************************************************************************************
        // x = balanceIn             x' = x +  virtualParamX                                         //
        // s = swapFee                                                                               //
        //                                                     1                                     //
...
class Gyro3Pool {
    ...
    static fromPool(pool) {
        ...
        if (parseFixed(pool.root3Alpha, 18).lte(0) ||
    ...
    getNormalizedLiquidity(poolPairData) {
        ...
        const normalisedLiquidity = _getNormalizedLiquidity(normalizedBalances, virtualOffsetInOut, poolPairData.swapFee);
        return bnum(formatFixed(normalisedLiquidity, 18));
    }
    getLimitAmountSwap(poolPairData, swapType) {
        if (swapType === SwapTypes.SwapExactIn) {
            return bnum(formatFixed(mulDown(poolPairData.balanceIn, this.MAX_IN_RATIO), poolPairData.decimalsIn));
        }
        else {
            return bnum(formatFixed(mulDown(poolPairData.balanceOut, this.MAX_OUT_RATIO), poolPairData.decimalsOut));
        }
    }
    // Updates the balance of a given token for the pool
    updateTokenBalanceForPool(token, newBalance) {
...
function getBoostedGraph(tokenIn, tokenOut, poolsAllDict, config) {
    ...
    for (const id in poolsAllDict) {
        ...
        for (const raisingToken of relevantRaisingTokens) {
            if (tokensList.includes(raisingToken) &&
                tokensList.includes(wethAddress) &&
...
export { LinearPool, MetaStablePool, BigNumber as OldBigNumber, PhantomStablePool, PoolFilter, PoolTypes, RouteProposer, SOR, stableMathBigInt as StableMathBigInt, stableMath as StableMaths, StablePool, SwapTypes, weightedMath as WeightedMaths, WeightedPool, ZERO, bnum, formatSequence, getSpotPriceAfterSwapForPath, getTokenAddressesForSwap, parseToPoolsDict, BPTForTokensZeroPriceImpact as phantomStableBPTForTokensZeroPriceImpact, queryBatchSwapTokensIn, queryBatchSwapTokensOut, BPTForTokensZeroPriceImpact$1 as stableBPTForTokensZeroPriceImpact, BPTForTokensZeroPriceImpact$2 as weightedBPTForTokensZeroPriceImpact };
